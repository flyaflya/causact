...
#' @param extraPara a vector of varible names to add in the model statement.  When \code{mcmc=FALSE}, the greta code is printed to the console, but not executed.  This option is most useful for posterior check purposes. When \code{mcmc=TRUE}, the code is executed in the global environment.
...
#' @export
dag_greta_20190522 <- function(graph,
                      mcmc = FALSE,
                      extraPara = NULL,
  ...
  unobservedNodes_dim = graph$nodes_df %>%
    dplyr::filter(obs == FALSE & distr == TRUE) %>%
    dplyr::inner_join(argDF,by=c("dimID"="rhsID")) %>%
    dplyr::filter(argName=='dim') %>%
    #dplyr::mutate(argDim = ifelse(is.na(argValue),0,argValue)) %>%
    dplyr::pull(argValue)

  unobservedNodes_dim = eval(rlang::parse_expr(unobservedNodes_dim[!is.na(unobservedNodes_dim)]))+sum(is.na(unobservedNodes_dim))
  ...
  if(is.null(extraPara)){
    ...
    posteriorStatement = paste0("draws_all       <- mcmc(",mcmcArgs,")   #POSTERIOR All
draws_all       <- replaceLabels(draws_all)   #POSTERIOR All
drawsDF_all     <- draws_all %>% as.matrix() %>% dplyr::as_tibble()   #POSTERIOR All
drawsDF         <- drawsDF_all[, (1:",unobservedNodes_dim,")] #POSTERIOR for unobserved parameters
draws_extraDF <- drawsDF_all[, -(1:",unobservedNodes_dim,")]   #POSTERIOR for extra parameters
tidyDrawsDF <- drawsDF %>% tidyr::gather() %>%
addPriorGroups()   #POSTERIOR for unobserved parameters\n")
  }
